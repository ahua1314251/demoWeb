...
import winker.sharding.DynamicDataSource;
 ...
 */
public abstract class WinkerDataSourceUtils {
	...
	private static final Log logger = LogFactory.getLog(WinkerDataSourceUtils.class);

	private static  ThreadLocal<DataSource> currentDataSource = new ThreadLocal<DataSource>();
	...
	public static Connection doGetConnection(DataSource dataSource) throws SQLException {
		...
		if(dataSource instanceof DynamicDataSource){
		   targetDataSource = ((DynamicDataSource)dataSource).getTargetDataSource();
		}else{
			targetDataSource = dataSource;
		}
		currentDataSource.set(targetDataSource);
		WinkerConnectionHolder conHolder = (WinkerConnectionHolder) TransactionSynchronizationManager.getResource(dataSource);
		if (conHolder != null && (conHolder.hasConnection() || conHolder.isSynchronizedWithTransaction())) {
			conHolder.requested();
			if (!conHolder.hasConnection()) {
				logger.debug("Fetching resumed JDBC Connection from DataSource");
				conHolder.setConnection(targetDataSource.getConnection(),targetDataSource);
			}
		...
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			WinkerConnectionHolder holderToUse = conHolder;
			if (holderToUse == null) {
				holderToUse = new WinkerConnectionHolder(con,targetDataSource);
			}
			else {
				holderToUse.setConnection(con,targetDataSource);
			}
	...
	public static void applyTimeout(Statement stmt, DataSource dataSource, int timeout) throws SQLException {
		...
		Assert.notNull(dataSource, "No DataSource specified");
		WinkerConnectionHolder holder = (WinkerConnectionHolder) TransactionSynchronizationManager.getResource(dataSource);
	 ...
	 * @param conHolder the WinkerConnectionHolder for the held Connection (potentially a proxy)
	 * @param passedInCon the Connection passed-in by the user
	 * (potentially a target Connection without proxy)
	 * @return whether the given Connections are equal
	 * @see #getTargetConnection
	 */
	private static boolean connectionEquals(WinkerConnectionHolder conHolder, Connection passedInCon) {
		if (!conHolder.hasConnection()) {
			return false;
		}
	...
	private static class ConnectionSynchronization extends TransactionSynchronizationAdapter {
		...
		private final WinkerConnectionHolder connectionHolder;
		...
		public ConnectionSynchronization(WinkerConnectionHolder connectionHolder, DataSource dataSource) {
		...
		public void suspend() {
			if (this.holderActive) {
				...
				if (this.connectionHolder.hasConnection() && !this.connectionHolder.isOpen()) {
					...
					// application accesses the WinkerConnectionHolder again after resume,
		...
		public void afterCompletion(int status) {
			...
			if (this.holderActive) {
				// The thread-bound WinkerConnectionHolder might not be available anymore,
				// since afterCompletion might get called from a different thread.
				TransactionSynchronizationManager.unbindResourceIfPossible(this.dataSource);
				this.holderActive = false;
				if (this.connectionHolder.hasConnection()) {
					releaseConnection(this.connectionHolder.getConnection(), this.dataSource);
					// Reset the WinkerConnectionHolder: It might remain bound to the thread.
					this.connectionHolder.close();;
				}
			}
	...
	public static ThreadLocal<DataSource> getCurrentDataSource() {
		return currentDataSource;
	}

	public static void setCurrentDataSource(ThreadLocal<DataSource> currentDataSource) {
		WinkerDataSourceUtils.currentDataSource = currentDataSource;
	}
